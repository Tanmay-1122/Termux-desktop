...
_set_panel_color() {
    ...
    local dark_mode_val="true"
    _xfconf_set xfce4-panel /panels/dark-mode bool "$dark_mode_val" >/dev/null 2>&1 || true
...
_force_xfce_dark_mode() {
    _xfconf_set xfsettings /Gtk/DarkTheme bool true >/dev/null 2>&1 || true
    _xfconf_set xsettings /Gtk/DarkTheme bool true >/dev/null 2>&1 || true
    _xfconf_set xfwm4 /general/theme "Default" >/dev/null 2>&1 || true
    _xfconf_set xfwm4 /general/button_layout "O|CHM" >/dev/null 2>&1 || true
    _xfconf_set xfwm4 /general/title_font "Sans 10" >/dev/null 2>&1 || true
    _xfconf_set xfce4-panel /panels/dark-mode bool true >/dev/null 2>&1 || true

    local xsettings_xml="$HOME/.config/xfce4/xfconf/xfce-perchannel-xml/xsettings.xml"
    if [ -f "$xsettings_xml" ]; then
        sed -i 's|<property name="DarkTheme" type="bool" value="false"/>|<property name="DarkTheme" type="bool" value="true"/>|g' "$xsettings_xml" 2>/dev/null || true
        sed -i 's|<property name="SyncThemes" type="bool" value="false"/>|<property name="SyncThemes" type="bool" value="true"/>|g' "$xsettings_xml" 2>/dev/null || true
    fi
}
...
_set_terminal() {
    ...
    local term_conf="$HOME/.config/xfce4/terminal/terminalrc"
    local accents_conf="$HOME/.config/xfce4/terminal/accels.scm"
...
ColorBackgroundType=0
ColorSelectionBackground=#000000
...
_hex_to_rgba() {
    local hex="${1//#/}"
    printf "rgba(%d, %d, %d, %s)" $((16#${hex:0:2})) $((16#${hex:2:2})) $((16#${hex:4:2})) "${2:-1}"
}
...
_set_gtk_css() {
    ...
    local bg_rgb fg_rgb accent_rgb panel_rgb
    bg_rgb=$(_hex_to_rgba_alpha "$bg")
    fg_rgb=$(_hex_to_rgba_alpha "$fg")
    accent_rgb=$(_hex_to_rgba_alpha "$accent")
    panel_rgb=$(_hex_to_rgba_alpha "$panel")

    if [ "$mode" = "light" ]; then
        border="rgba(20, 24, 35, 0.12)"
        shadow="rgba(20, 24, 35, 0.10)"
    else
        border="rgba(255, 255, 255, 0.08)"
        shadow="rgba(0, 0, 0, 0.35)"
    fi

    # Try template file first, fall back to inline sed
    local template="$PREFIX/share/termux-desktop/gtk-base.css"
    if [ -f "$template" ]; then
        sed \
            ...
            -e "s/@PANEL_RGB@/${panel_rgb}/g" \
            "$template" > "$css3"
    else
        # Inline fallback (minimal — just set define-color, GTK handles the rest)
        cat > "$css3" << CSSEOF
/* Termux Desktop theme: ${name} */
@define-color theme_bg ${bg};
@define-color theme_fg ${fg};
@define-color theme_panel ${panel};
@define-color theme_accent ${accent};
@define-color theme_border ${border};
CSSEOF
    fi
    cp "$css3" "$css4" 2>/dev/null || true
    ok "GTK theme styling applied"
}
...
_set_wallpaper_zoom_fill() {
    local monitor base
    for monitor in monitor0 monitorVNC-0 monitorbuiltin monitorDexDisplay; do
        base="/backdrop/screen0/${monitor}/workspace0"
        _xfconf_set xfce4-desktop "${base}/image-style" int 3 >/dev/null 2>&1 || true
        _xfconf_set xfce4-desktop "${base}/color-style" int 0 >/dev/null 2>&1 || true
        _xfconf_set xfce4-desktop "${base}/color1" string "#000000" >/dev/null 2>&1 || true
    done
}
...
echo "       Termux Desktop — Theme Switcher v${PROJECT_VER}"
echo "=================================================="
echo ""
echo "  DARK THEMES:"
